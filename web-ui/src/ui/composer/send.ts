...
export function sendComposer(): void {
    ...
    try {
        // Get files that need to be injected
        const filesToInject = getNewlyCheckedFiles()

        // Use unified send that combines text, files, and history in one message
        if (normalized || filesToInject.length > 0) {
            sendTextAndFilesWithHistory(normalized, filesToInject, historyEntry)
        }

        // Clean up file state after sending (same as finalizeIterationAndSendEnter but without the separate injectFiles call)
        makePermanentAndCleanup(filesToInject)
