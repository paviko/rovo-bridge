...
import {getNewlyCheckedFiles, makePermanentAndCleanup, sendTextAndFilesWithHistory, sendTextToStdin} from '../session'
...
vi.mock('../session', () => ({
    getNewlyCheckedFiles: vi.fn(() => []),
    makePermanentAndCleanup: vi.fn(),
    sendTextAndFilesWithHistory: vi.fn(),
...
describe('sendComposer', () => {
    ...
    it('should capture prompt history when sending composer content', () => {
        ...
        // Verify text was sent with history using the new unified function
        expect(sendTextAndFilesWithHistory).toHaveBeenCalledWith(
        ...
        // Verify file processing and cleanup was called
        expect(getNewlyCheckedFiles).toHaveBeenCalled()
        expect(makePermanentAndCleanup).toHaveBeenCalled()
        expect(sendTextToStdin).toHaveBeenCalledWith('\r')
    })
    ...
    it('should not capture history when history is not initialized', () => {
        ...
        expect(sendTextAndFilesWithHistory).toHaveBeenCalledWith('Test content', [], null)
    })
    ...
    it('should not capture history for empty content', () => {
        ...
        expect(promptHistoryManager.addPromptWithId).not.toHaveBeenCalled()
        expect(sendTextAndFilesWithHistory).not.toHaveBeenCalled()
    })
    ...
    it('should handle history capture errors gracefully', () => {
        ...
        expect(sendTextAndFilesWithHistory).toHaveBeenCalled()
    })
})
