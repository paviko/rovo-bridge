...
export function registerDefaultHandlers(ctx: HandlerCtx): void {
    ...
    ctx.registerHandler('updateUseClipboard', (message) => {
        try {
            // Cache preference regardless of WS state; used in openSession
            const useClipboard = !!message.useClipboard;
            (state as any).useClipboardPref = useClipboard;

            // Forward to backend immediately if WS is open
            const ws = state.currentWs;
            if (ws && ws.readyState === WebSocket.OPEN) {
                const sid = (window as any).__SESSION_ID__ || 's1';
                ws.send(JSON.stringify({type: 'updateUseClipboard', sessionId: sid, useClipboard}));
                if (isDev) console.log(`[MessageDispatcher] updateUseClipboard forwarded to backend: ${useClipboard}`);
            } else {
                if (isDev) console.log('[MessageDispatcher] Cached updateUseClipboard; will be applied on session open');
            }
        } catch (error) {
            console.error('[MessageDispatcher] updateUseClipboard handler error:', error);
        }
    });
