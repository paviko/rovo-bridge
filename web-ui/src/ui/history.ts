...
export class PromptHistoryManager {
  ...
  getFilteredHistory(filter: HistoryFilter): PromptHistoryEntry[] {
    try {
      ...
      // Return all entries if showing all projects
      ...
      if (!targetCwd) {
        console.warn('No target CWD available for filtering, returning all entries')
        return [...this.cache]
      }

      // Filter with error handling for individual entries
      const filteredEntries: PromptHistoryEntry[] = []
      let errorCount = 0

      for (const entry of this.cache) {
        try {
          if (this.isProjectMatch(entry.projectCwd, targetCwd)) {
            filteredEntries.push(entry)
          }
        } catch (entryError) {
          console.warn('Error processing history entry during filtering:', entryError, entry)
          errorCount++
        }
      }

      if (errorCount > 0) {
        console.warn(`Encountered ${errorCount} errors while filtering history entries`)
      }
