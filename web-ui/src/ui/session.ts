...
export function sendTextToStdinWithHistory(text: string, historyEntry: any) {
    _sendMessageWithHistory('stdin', text, [], historyEntry)
}

// New unified send function that combines text and files in one message
export function sendTextAndFilesWithHistory(text: string, paths: string[], historyEntry: any) {
    _sendMessageWithHistory('send', text, paths, historyEntry)
}

// Internal helper function to handle sending messages with history and error recovery
function _sendMessageWithHistory(messageType: 'stdin' | 'send', text: string, paths: string[], historyEntry: any) {
    if (!state.currentWs || state.currentWs.readyState !== WebSocket.OPEN) {
        console.warn(`Cannot send ${messageType} message: WebSocket not connected`)
        return
    }
    ...
    try {
        ...
        if (typeof text !== 'string') {
            console.error(`Invalid text parameter for ${messageType}:`, typeof text)
            return
        }
        ...
        // Add paths for 'send' message type
        if (messageType === 'send' && paths && paths.length > 0) {
            message.paths = paths
        }
        ...
        console.error(`Failed to send ${messageType} message with history:`, e)

        // Priority: ensure history is saved even if message send fails
        const sid = (window as any).__SESSION_ID__ || 's1'
        try {
        ...
        // Attempt to still send the message payload (without history) as best-effort
        try {
            if (state.currentWs && state.currentWs.readyState === WebSocket.OPEN) {
                ...
                // Add paths for 'send' message fallback
                if (messageType === 'send' && paths && paths.length > 0) {
                    fallbackMessage.paths = paths
                }

                state.currentWs.send(JSON.stringify(fallbackMessage))
                console.log(`Sent ${messageType} message after savePrompt fallback`)
            }
        } catch (fallbackError) {
            console.error(`Fallback ${messageType} send also failed:`, fallbackError)
        }
    }
}
...
export function getNewlyCheckedFiles(): string[] {
    ...
    return newlyChecked.filter((p) => !toRemove.has(p))
}

// Make newly checked files permanent and clean up state
export function makePermanentAndCleanup(filteredNewlyChecked: string[]) {
...
export function finalizeIterationAndSendEnter(): void {
    try {
        const filteredNewlyChecked = getNewlyCheckedFiles()
        makePermanentAndCleanup(filteredNewlyChecked)
